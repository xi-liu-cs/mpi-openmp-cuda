...
__global__ void vecGPU(float *ad, float *bd, float *cd, int cycles, int step);
...
int main(int argc, char *argv[])
{
	...
	*/
	...
	int cycles = ceil(ceil((double)n / nTid) / nBlk);
	int step = nBlk * nTid;
	vecGPU<<<nBlk, nTid>>>(ad, bd, cd, cycles, step);
...
__global__ void vecGPU(float *ad, float *bd, float *cd, int cycles, int step)
{
	...
	{
		int i = blockIdx.x * blockDim.x + threadIdx.x + step * j;
		cd[i] += ad[i] * bd[i];
	}
}